...
| mu-plugin (aix-seo-suite.php) | **mu-plugin設置は却下**（誤情報多数） → **FSEが手動で同等施策を実装** | canonical・構造化データ等はFSEが順次対応 |
...
| スキル | 関係 | 使い所 |
|--------|------|--------|
| `/article-skills` | 相互参照 | 記事の新規生成はarticle-skills、品質改善・AIEO最適化はこのスキル |
| `/yt-pipeline` | 後続 | YT記事もこのスキルの品質ルールに従う |
